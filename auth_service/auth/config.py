...
class AuthConfig(BaseModel):
    ...
    jwt_private_key: str = Field(
        ..., description="PEM-encoded RSA private key used to sign access tokens"
    )
    jwt_public_key: str = Field(
        ..., description="PEM-encoded RSA public key used to verify access tokens"
    )
    secret_key: SecretStr = Field(
        ..., description="High-entropy application secret used for symmetric encryption"
    )
    ...
    @classmethod
    def normalize_token_scope(cls, value: str) -> str:
        value = value.strip()
        if not value:
