...
def get_auth_router(auth_service_dep: Callable[..., AuthService]) -> APIRouter:
    ...
    async def register(
        payload: UserCreate, auth: AuthService = Depends(auth_service_dep)
    ) -> User:
        try:
    ...
    async def login(
        payload: UserLogin, auth: AuthService = Depends(auth_service_dep)
    ) -> TokenResponse:
        try:
    ...
    async def refresh(
        payload: RefreshRequest, auth: AuthService = Depends(auth_service_dep)
    ) -> TokenResponse:
        try:
