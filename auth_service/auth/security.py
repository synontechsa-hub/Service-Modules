...
class SecurityManager:
    ...
    def _load_and_validate_jwt_keys(self) -> tuple[rsa.RSAPrivateKey, rsa.RSAPublicKey]:
        try:
            private_key = serialization.load_pem_private_key(
                self.config.jwt_private_key.encode("utf-8"), password=None
            )
    ...
    def _build_fernet(self) -> Fernet:
        hkdf = HKDF(
            ...
            length=32,
            salt=b"auth-module-fernet-v1",
            info=b"auth-module-encryption",
        )
    ...
    def create_access_token(
        self, user_id: uuid.UUID, extra_claims: dict[str, Any] | None = None
    ) -> tuple[str, int]:
        ...
        return jwt.encode(payload, self._private_key, algorithm="RS256"), int(
    ...
    def decode_access_token(self, token: str) -> dict[str, Any]:
        try:
            payload = jwt.decode(
                ...
                options={"require": list(_RESERVED_CLAIMS)},
            )
