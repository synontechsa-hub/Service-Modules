...
class AuthService:
    def __init__(
        ...
        if security is not None and security.config != config:
            raise ValueError("Injected SecurityManager must use the same AuthConfig")
        self.config = config
        self.db = db
        self.security = security or SecurityManager(config)
    ...
    async def login(self, payload: UserLogin) -> TokenResponse:
        ...
        result = await self.db.execute(
            select(User)
            .where(or_(User.username == identifier, User.email == identifier.lower()))
            .with_for_update()
        )
        user = result.scalar_one_or_none()
        if user is None or user.password_hash is None:
            await asyncio.to_thread(self.security.burn_password_check, payload.password)
            await self.db.rollback()
            raise InvalidCredentialsError("Invalid credentials.")

        now = datetime.now(timezone.utc)
        is_locked = bool(user.locked_until and _utc(user.locked_until) > now)
        ...
        if not password_valid:
            if is_locked or not user.is_active:
                await self.db.rollback()
                raise InvalidCredentialsError("Invalid credentials.")
