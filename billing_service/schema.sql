...
create table if not exists plans (
    ...
    frequency smallint not null default 3,  -- 3=monthly 4=quarterly 5=biannual 6=annual
    cycles smallint not null default 0,      -- 0 = indefinite
...
-- Example: seed a couple of plans to get started.
-- insert into plans (code, name, amount, frequency, cycles) values
--     ('starter_monthly', 'Starter', 99.00, 3, 0),
--     ('pro_monthly', 'Pro', 299.00, 3, 0);

-- Same reasoning as the auth service: this connects directly via Postgres,
-- not through Supabase's PostgREST API, so lock these tables out of that
-- API entirely. Only a role with BYPASSRLS (the one this service connects
-- as) can reach them.
